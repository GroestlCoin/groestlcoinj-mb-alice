...
import com.google.groestlcoin.IsMultiBitClass;
...
import com.google.groestlcoin.store.UnreadableWalletException;
import com.google.groestlcoin.store.WalletProtobufSerializer;
import com.google.groestlcoin.utils.ListenerRegistration;
import com.google.groestlcoin.utils.Threading;
import com.google.groestlcoin.wallet.KeyTimeCoinSelector;
import com.google.groestlcoin.wallet.WalletFiles;
...
import static com.google.groestlcoin.core.Utils.bitcoinValueToFriendlyString;
 ...
 * {@link Wallet#autosaveToFile(java.io.File, long, java.util.concurrent.TimeUnit, com.google.groestlcoin.wallet.WalletFiles.Listener)}
...
public class Wallet implements Serializable, BlockChainListener, PeerFilterProvider, IsMultiBitClass {
    ...
    public void receivePending(Transaction tx, @Nullable List<Transaction> dependencies, boolean overrideIsRelevant) throws VerificationException {
        ...
        try {
            ...
            if (log.isInfoEnabled()) {
                log.info(String.format("Received a pending transaction %s that spends %s GRS from our own wallet (%s)," +
                        " and sends us %s GRS", tx.getHashAsString(), Utils.bitcoinValueToFriendlyString(valueSentFromMe),
    ...
    private void receive(Transaction tx, StoredBlock block, BlockChain.NewBlockType blockType,
        ...
        log.info("Received tx{} for {} GRS: {} [{}] in block {}", new Object[]{sideChain ? " on a side chain" : "",
    ...
    public static class SendRequest {
         ...
         * (its value is ignored and set to {@link com.google.groestlcoin.core.Wallet#getBalance()} - the fees required
         ...
         *
         * <p>Once {@link Wallet#completeTx(com.google.groestlcoin.core.Wallet.SendRequest)} is called, this is set to the
    ...
    public boolean completeTx(SendRequest req, boolean sign) {
        ...
        try {
            ...
            // We need to know if we need to add an additional fee because one of our values are smaller than 1 GRS
            boolean needAtLeastReferenceFee = false;
            int txOutDustFeeCount = 0;
            if (req.ensureMinRequiredFee && !req.emptyWallet) { // min fee checking is handled later for emptyWallet
                for (TransactionOutput output : req.tx.getOutputs())
                    if (output.getValue().compareTo(Utils.COIN) < 0) { //TXOut lower than 1 GRS have a 2 GRS fee!
                        //TODO Currently Groestlcoin doesn't have this. We can put it back in later.
...
//                            throw new IllegalArgumentException("Tried to send dust with ensureMinRequiredFee set - no way to complete this");
                        ...
                        txOutDustFeeCount++; //GRS: Each TXOut < 1 GRS needs +1 GRS fee!
                    }
            }
     ...
     * If {@link Wallet#autosaveToFile(java.io.File, long, java.util.concurrent.TimeUnit, com.google.groestlcoin.wallet.WalletFiles.Listener)}
    ...
    public String toString(boolean includePrivateKeys, boolean includeTransactions, boolean includeExtensions, AbstractBlockChain chain) {
        ...
        try {
            ...
            builder.append(String.format("Wallet containing %s GRS in:%n", bitcoinValueToFriendlyString(getBalance())));
     ...
     * {@link com.google.groestlcoin.core.ECKey#getCreationTimeSeconds()}. This can return zero if at least one key does
    ...
    /**
     * Convenience wrapper around {@link Wallet#encrypt(com.google.groestlcoin.crypto.KeyCrypter,
     ...
     * {@link com.google.groestlcoin.crypto.KeyCrypterScrypt}.
     *
        ...
        try {
            ...
            for (ECKey key : keychain) {
                if (key.isEncrypted()) {
                    ...
                    // (all groestlcoin controlled by that private key is lost forever).
    ...
    /**
     * <p>Convenience wrapper around {@link Wallet#addNewEncryptedKey(com.google.groestlcoin.crypto.KeyCrypter,
     ...
     * you can use {@link com.google.groestlcoin.utils.Threading#waitForUserCode()} to block until the future had a
    ...
    private class FeeCalculation {
        ...
        public FeeCalculation(SendRequest req, BigInteger value, List<TransactionInput> originalInputs,
            ...
            while (true) {
                ...
                //GRS: Add 1 GRS fee per txOut < 1 GRS
                ...
                // If change is < 1 GRS, we will need to have at least minfee to be accepted by the network
                ...
                if (change.compareTo(BigInteger.ZERO) > 0) {
                    ...
                    if (req.ensureMinRequiredFee && BigInteger.valueOf(100000000).compareTo(change) > 0) {
                        ...
                        //Throw away change lower than 1 GRS as this is cheaper than paying the 1 GRS fee.
                        isCategory3 = true;
                        ...
                        additionalValueForNextCategory = Transaction.REFERENCE_DEFAULT_MIN_TX_FEE.add(BigInteger.ONE);
                                                         //GRS: We don't have a min value, but we add more fees for tx < 1
                    } else {
     ...
     */
    public void setTransactionBroadcaster(@Nullable com.google.groestlcoin.core.TransactionBroadcaster broadcaster) {
        lock.lock();
        try {
