 ...
 *
 * <ul>
 * <li>{@code groestlcoin:<address>}</li>
 * <li>{@code groestlcoin:<address>?<name1>=<value1>&<name2>=<value2>} with multiple
 ...
 * @see <a href="https://en.groestlcoin.it/wiki/BIP_0021">BIP 0021</a>
 */
public class BitcoinURI {
    ...
    public BitcoinURI(@Nullable NetworkParameters params, String input) throws BitcoinURIParseException {
        ...
        // URI is formed as  groestlcoin:<address>?<query parameters>
        // blockchain.info generates URIs of non-BIP compliant form groestlcoin://address?....
        // We support both until Ben fixes his code.
        
        // Remove the groestlcoin scheme.
        // (Note: getSchemeSpecificPart() is not used as it unescapes the label and parse then fails.
        // For instance with : groestlcoin:129mVqKUmJ9uwPxKJBnNdABbuaaNfho4Ha?amount=0.06&label=Tom%20%26%20Jerry
        // the & (%26) in Tom and Jerry gets interpreted as a separator and the label then gets parsed
        // as 'Tom ' instead of 'Tom & Jerry')
        String schemeSpecificPart;
        if (input.startsWith("groestlcoin://")) {
            schemeSpecificPart = input.substring("groestlcoin://".length());
        } else if (input.startsWith("groestlcoin:")) {
            schemeSpecificPart = input.substring("groestlcoin:".length());
        } else {
